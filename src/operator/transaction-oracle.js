...
module.exports = class TransactionOracle {
	constructor() {
	...
	register = async (queryFee = 200000000000000) => {
		...
		if (!this.oracle)
			this.oracle = await this.aeternity.client
				.getOracleObject(this.aeternity.keypair.publicKey.replace("ak_", "ok_"))
				.catch(() => null);
		if (!this.oracle)
	...
	respond = async (queries) => {
		...
		const url = `http://localhost:4300/api/v1/transactions/${queryString}`;
		...
		if (processorResponseText.toLowerCase() === "approved") {
			...
			await this.oracle.respondToQuery(
				query.id,
				processorResponseText.toLowerCase(),
				{
